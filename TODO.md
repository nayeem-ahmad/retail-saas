...
- [x] Fix failing CI/CD pipeline that blocked Render deploys (backend + frontend) — commit `0d0ce5f` (product price lists) added `PriceListsService` as the 3rd constructor dependency of `ProductsService` (wired correctly via `PriceListsModule` import/export) but did not update `products.service.spec.ts`, so the `test-backend` job failed with a Nest dependency-resolution error. Since Render deploys gate on the shared CI/CD Pipeline passing, the single failing backend unit test blocked both service deploys. Fix: added a mocked `PriceListsService` provider (`addProductToAllActiveLists`) + import to the spec's test module. App build/boot were never broken — verified spec passes (8/8) — done 2026-06-17
